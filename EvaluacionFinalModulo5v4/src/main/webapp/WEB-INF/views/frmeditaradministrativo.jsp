...
<br/>
 <div class="container">
    ...
    <!--<c:forEach items="${ladministrativo}" var="la">  </c:forEach> -->
    	<form action='<c:out value= "${pageContext.request.contextPath}"/>/procesaeditaradministrativo' method="POST">
			<div class="mb-3">
				<label for="runadmin" class="form-label">Run Administrador</label> 
				<input type="text" class="form-control" id="runadmin"  value='<c:out value="${myadm.getRunadmin()}"/>'  name="runadmin" placeholder="123456789">
			</div>
			<div class="mb-3">
				<label for="nombres" class="form-label">Nombres</label> 
				<input type="text" class="form-control" id="nombres" value='<c:out value="${myadm.getNombres()}"/>' name="nombres" placeholder="Sebastian Luis">
			</div>
			<div class="mb-3">
				<label for="apellidos" class="form-label">Apellidos</label> 
				<input type="text" class="form-control" id="apellidos" value='<c:out value="${myadm.getApellidos()}"/>' name="apellidos" placeholder="Lopez Perez">
			</div>
			<div class="mb-3">
				<label for="email" class="form-label">Email</label> 
				<input type="email" class="form-control" id="email" value='<c:out value="${myadm.getEmail()}"/>' name="email" placeholder="seba.lo@gmail.com">
			</div>
			<div class="mb-3">
				<label for="area" class="form-label">Area</label> 
				<input type="text" class="form-control" id="area" value='<c:out value="${myadm.getArea()}"/>' name="area" placeholder="Finanzas">
			</div>
			<div class="mb-3">
				<label for="usuario_runusu" class="form-label">Rut usuario</label> 
				<input type="text" class="form-control" id="usuario_runusu" value='<c:out value="${myadm.getUsuario_runusu()}"/>' name="usuario_runusu">
			</div>

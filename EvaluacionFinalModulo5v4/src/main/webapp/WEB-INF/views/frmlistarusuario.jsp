		...
		<table class="table">
			<thead class="table-dark">
				<tr>
					...
					<th scope="col">Run</th>
					<th scope="col">Editar tipo cliente</th>
			...
			<tbody>
				<c:forEach items="${lusuario}" var="lu">
					<tr>
						...
						<td><c:out value="${lu.getRunusu()}" /></td>
						<!--<td><c:out value="${lu.getTipo()}" /></td>
						<!--  <td><a href="${pageContext.request.contextPath}/editar/${lu.getTipo()}" title="Editar"><c:out value="${lu.getTipo()}" ></c:out></a></td>-->
						<td>
						
						
						<c:if test="${lu.getTipo()=='Cliente'}" >
							<a href='<c:out value="${pageContext.request.contextPath}" />/editarcliente/<c:out value="${lu.getRunusu()}" />'>
							<button class="btn btn-primary">${lu.getTipo()}</button></a>
						</c:if>
						
						<c:if test="${lu.getTipo()=='Profesional'}" >
							<a href='<c:out value="${pageContext.request.contextPath}" />/editarprofesional/<c:out value="${lu.getRunusu()}" />'>
							<button class="btn btn-primary">${lu.getTipo()}</button></a>
						</c:if>
						
						<c:if test="${lu.getTipo()=='Administrativo'}" >
							<a href='<c:out value="${pageContext.request.contextPath}" />/editaradministrativo/<c:out value="${lu.getRunusu()}" />'>
							<button class="btn btn-primary">${lu.getTipo()}</button></a>
						</c:if>
						...
						<td>
							<a href='<c:out value="${pageContext.request.contextPath}" />/eliminarusuario/<c:out value="${myusu.getRun()}" />'><i class="fas fa-trash-alt"  title="Eliminar"></i></a>
							&nbsp; &nbsp; &nbsp; | &nbsp; &nbsp; &nbsp;  
							<a href='<c:out value="${pageContext.request.contextPath}" />/editarusuario/<c:out value="${myusu.getRun()}" />'><i class="fas fa-user-edit"  title="Editar"></i></a>
																							
						</td>
					</tr>
				</c:forEach>
				<c:forEach items="z" >
					<c:out value="${lu.getRunusu()}"/>
				</c:forEach>
				
				
			</tbody>
		</table>
		<a href= '<c:out value= "${pageContext.request.contextPath}"/>/crearusuario' class="btn btn-primary">Crear usuario</a>
	</div>
	<br/>
	<br/>
